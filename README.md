...
- Celery worker (Redis broker) executing a 5-second simulated external API call and logging "Order #ID Processed." when complete.
...
This project is split into a synchronous API layer and an asynchronous processing layer:

- **Django + DRF (API)** handles product CRUD and purchase requests.
- **PostgreSQL** is the source of truth for product stock and orders.
- **Redis** is used in two ways:
  - **Cache** for product detail (`GET /api/products/{id}/`) to reduce database load.
  - **Celery broker/result backend** to enqueue background jobs and store results.
- **Celery worker** processes orders asynchronously (simulated external API call), updates order status, and appends order logs.

## Race Condition Handling (Stock Consistency)

![Race Condition Handling](assets/rice-condition-handler.png)

In an e-commerce system, stock consistency becomes tricky under concurrency. For example, imagine a product has **stock = 1**, and **two (or more) users** submit a purchase at nearly the same time. The system must ensure that **at most one** purchase succeeds and the others fail gracefully (instead of overselling).

### Problem: non-atomic “read → compute → write” (no lock)
If multiple requests do something like:

1. `SELECT stock FROM product WHERE id = X`
2. Check `stock >= quantity`
3. `UPDATE product SET stock = stock - quantity`

…then concurrent requests can read the same stock value and both pass the check, causing **lost updates** and **overselling**.

### Solution: atomic conditional update (single SQL statement)
This project prevents the race condition by using a single atomic statement:
...
Why this works in PostgreSQL:
- The `UPDATE` acquires a **row lock** on the matched product row.
- The `stock >= quantity` condition is checked **under the same lock**.
- Only one transaction can successfully update the “last stock” case.
- The API checks affected rows (`updated_rows`); if it is `0`, it returns **409 Out of stock**.

## End-to-End Flow

1. **Client hits `/api/orders/` (POST)** with `product_id` and `quantity`.
2. **Atomic stock decrement** in `purchase_product` service (see **Race Condition Handling (Stock Consistency)** above).
...
└─ assets/
   ...
   ├─ rice-condition-handler.png # Race condition illustration
   └─ erd.png                    # ERD diagram
```
